	...
	<body>
		...
		<div id="mainbody">
			...
			<div id="bottom">
				...
				if(mysql_num_rows(mysql_query("SELECT * FROM `users1`.`book` WHERE `book`.`venue` = '$venue' AND `book`.`date` = '$date' AND `book`.`from` = '$from'"))){
					$msg=$msg."Another Event booked at this time in $venue. Try again please.<br/>";
					$status= "NOTOK";}

				if($status<>"OK"){
				echo "<font face='Verdana' size='2' color=red>$msg</font><br><center><input type='button' value='Retry' onClick='history.go(-1)'></center>";
				}else{ // if all validations are passed.
				$query=mysql_query("INSERT INTO `users1`.`book` (`purpose`, `venue`, `date`, `from`) VALUES ('$purpose', '$venue', '$date', '$from')");
				if($query){echo "<font face='Verdana' size='2' color=green>Welcome, You have successfully booked the event.<br><br>The detailes are:<br/>Purpose: $purpose<br/>Date: $date<br/>Time: $from<br/> Duration: 1 hour<br/> Venue: $venue<br/><a href=index.php>Click here to go back to your home page.</a><br></font>";}
				}
				}
				?>
				</div>
